...
class Trainer:
    ...
    def __init__(self, work_dir, depth, NN, device, batchsize=50, dropout=0.5, lr=0.01, seed=666, naugment=0):
        ...
        # model
        self.model = SqueezeUNet(NN=self.NN, depth=self.depth ,wf=6, dropout=self.dropout).to(self.device)
        ...
        self.train_dataset   = CustomDataset(NN=cfg.DN_NN, transform=self.transform, augment=True, naugment=self.naugment)
        self.valid_dataset   = CustomDataset(NN=cfg.DN_NN, transform=self.transform, augment=False, naugment=0)
        ...
        self.optimizer_scheduler = lr_scheduler.LambdaLR(self.optimizer, lr_lambda=lambda1)
        ...
        # summary
        num_parameters = sum(p.numel() for p in self.model.parameters() if p.requires_grad) / 1.e6
    ...
    def init_dataloaders(self):
        self.train_loader    = torch.utils.data.DataLoader(self.train_dataset, batch_size=self.batchsize, prefetch_factor=6, num_workers=12, pin_memory=True, drop_last=True, shuffle=True)
        self.valid_loader    = torch.utils.data.DataLoader(self.valid_dataset, batch_size=self.batchsize, prefetch_factor=6, num_workers=12, pin_memory=True, drop_last=True, shuffle=False)
    ...
    def optimize(self):
        ...
        for batch_id, (targets, inputs) in enumerate(self.train_loader):
            ...
            if (batch_id + 1) % 10 == 0:
                ...
                print('train loss @batch_id {}/{}: {} in {} mins {} secs / batch'.format(str(batch_id+1).zfill(len(str(len(self.train_loader)))), len(self.train_loader), loss.item(), int(m), round(s, 2)), flush=True)
        ...
        h,m = divmod(m, 60)
        print('epoch training time: {} hrs {} mins {} secs'.format(int(h), int(m), int(s)))
        ...
        with torch.no_grad():
            for batch_id, (targets, inputs) in enumerate(self.valid_loader):
                ...
                if (batch_id + 1) % 10 == 0:
                    print('val loss   @batch_id {}/{}: {}'.format(str(batch_id+1).zfill(len(str(len(self.valid_loader)))),
                        len(self.valid_loader), loss.item()),flush=True)
        ...
        h,m = divmod(m, 60)
        print('epoch validation time: {} hrs {} mins {} secs'.format(int(h), int(m), int(s)), flush=True)

        # Logging
        if self.epoch % self.freq_write_model == 0:
            self.write_model(self.models_dir + 'model{}.pth'.format(str(self.epoch-1).zfill(6)))
    ...
    def reconstruct(self, noisy_ref, noisy_imgs, filename_rec):
        if len(noisy_imgs) != self.NN:
            print('The model was not initiated with the correct number of neighbouring slices')
            sys.exit(-1)
        ...
        aligned_ref, aligned_imgs = aligner.align_images(noisy_ref, noisy_imgs, align_patch=False)
        ...
        batch      = np.zeros((self.batchsize,self.NN,SUBTILE_SIZE,SUBTILE_SIZE),dtype=np.float32)
        batch_out  = np.zeros((self.batchsize, 1,SUBTILE_SIZE,SUBTILE_SIZE),dtype=np.float32)
        ...
        for itile in tqdm.tqdm(range(0,ntiles,self.batchsize)):
            ...
            for i in range(nbatch):
                ...
                batch[i,:,:,:] = mics[:,ix:ix+SUBTILE_SIZE,iy:iy+SUBTILE_SIZE]
            ...
            with torch.no_grad():
                batch_out[:nbatch,:,:,:] = self.model(noisy_imgs).detach().cpu().numpy()
            ...
            for i in range(nbatch):
                ix = tiles_coords[ibatch_start+i][0]
                iy = tiles_coords[ibatch_start+i][1]
                mic_out[ix+BORDER:ix+SUBTILE_SIZE-BORDER,iy+BORDER:iy+SUBTILE_SIZE-BORDER]  += mask.D * batch_out[i,0,BORDER:SUBTILE_SIZE-BORDER,BORDER:SUBTILE_SIZE-BORDER]
                mic_norm[ix+BORDER:ix+SUBTILE_SIZE-BORDER,iy+BORDER:iy+SUBTILE_SIZE-BORDER] += mask.D
