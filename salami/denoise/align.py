...
class Aligner:
    ...
    def __align_tiles(self, img1, img2, x_range, y_range):
        ...
        for i in x_range:
            ...
            for j in y_range:
                ...
                self.tile1.D[:,:] = img1.D[i:i+self.AL_TILESZ,j:j+self.AL_TILESZ] 
                self.tile2.D[:,:] = img2.D[i:i+self.AL_TILESZ,j:j+self.AL_TILESZ]
                ...
                cc, offset = self.tile1.get_best_shift(self.tile2, normalized=False, peakInterpolation=True)
                x[ni,nj], y[ni,nj] = pix2coords(i+self.AL_TILESZ/2.0,j+self.AL_TILESZ/2.0,img1.nx,img1.ny)
                sx[ni,nj] = offset[0]
                sy[ni,nj] = offset[1]
        ...
        X  = x.flatten()
        Y  = y.flatten()
        A  = np.array([X*0.0+1.0, X, Y, X*Y, X**2, Y**2, X**2*Y, X*Y**2]).T
    ...
    def align_images(self, ref_filename, imgs_list, align_patch=True):
        ...
        fname_ref_out = '%s/%s%06d_ref.tif' % (self.aligned_dir, self.PREFIX, self.AL_GLOBAL_COUNTER)
        reference_bak = reference.copy()
        reference_bak.write(fname_ref_out,mode='uint8')

        # prep for alignement, interpolation
        reference     = reference.clip(self.AL_BORDER,self.AL_BORDER,self.AL_BORDER,self.AL_BORDER)
        reference.normalize()
        ...
        for img_fname in imgs_list:
            N += 1
            output_name = '%s/%s%06d_%1d.tif' % (self.aligned_dir, self.PREFIX, self.AL_GLOBAL_COUNTER, N)

            # read & image prep
            frame     = Frame(filename=img_fname)
            frame_bak = frame.copy()
            frame     = frame.clip(self.AL_BORDER,self.AL_BORDER,self.AL_BORDER,self.AL_BORDER)
            frame.normalize()
            ...
            cc, offset     = frame_crop.get_best_shift(reference_crop, normalized=False, peakInterpolation=True)
            offset        /= scaling_eff
            ...
            if align_patch:
                ...
                x_range  = range(border_x, int(frame_bak.nx-border_x-self.AL_TILESZ), self.AL_TILE_DENSITY)
                y_range  = range(border_y, int(frame_bak.ny-border_y-self.AL_TILESZ), self.AL_TILE_DENSITY)

                # Local Alignment
                coeffs_x, coeffs_y = self.__align_tiles(frame_bak, reference_bak, x_range, y_range)
                set_pyfttw_nthreads(self.NTHREADS)
    ...
    def generate_denoising_tiles(self, reference, noisy_inputs, blur_reference=True, valid_frac=0.2):
        self.DN_GLOBAL_COUNTER += 1
        ...
        for i in range(ntx):
            ...
            for j in range(nty): 
                pj = j*self.DN_TILESZ
                n += 1
                ref_tiles[n,:,:] = np.rot90(ref.D[pi:pi+self.DN_TILESZ,pj:pj+self.DN_TILESZ],1)
        ...
        for i in range(nt):
            if vec[i] < 0.5:
                if blur_reference:
                    ...
                    ref_train_tiles.append(Image.fromarray(ref_tiles[i,:,:].astype('uint8')))
            else:
                if blur_reference:
                    ref_valid_tiles.append(Image.fromarray(ref_tiles[i,:,:]))
                else:
                    ref_valid_tiles.append(Image.fromarray(ref_tiles[i,:,:].astype('uint8')))

        # Write reference stack
        train_ref_name = self.train_dir+'train_'+str(self.DN_GLOBAL_COUNTER)+'_ref.tif'
        valid_ref_name = self.valid_dir+'valid_'+str(self.DN_GLOBAL_COUNTER)+'_ref.tif'
        ref_train_tiles[0].save(train_ref_name, compression=None, save_all=True, append_images=ref_train_tiles[1:])
        ref_valid_tiles[0].save(valid_ref_name, compression=None, save_all=True, append_images=ref_valid_tiles[1:])
        ...
        for noisy_input in noisy_inputs:
            ...
            frame        = frame.clip(self.DN_BORDER,self.DN_BORDER,self.DN_BORDER,self.DN_BORDER)
            inp_tiles    = np.ndarray((nt,self.DN_TILESZ,self.DN_TILESZ),dtype=np.float32)
            ...
            for i in range(ntx):
                ...
                for j in range(nty): 
                    pj = j*self.DN_TILESZ
                    n += 1
                    inp_tiles[n,:,:] = np.rot90(frame.D[pi:pi+self.DN_TILESZ,pj:pj+self.DN_TILESZ],1)
            inp_valid_tiles = []
            inp_train_tiles = []
            for i in range(nt):
                if vec[i] < 0.5:
                    inp_train_tiles.append(Image.fromarray(inp_tiles[i,:,:].astype('uint8')))
                else:
                    inp_valid_tiles.append(Image.fromarray(inp_tiles[i,:,:].astype('uint8')))
            # Write input stack
            train_name = self.train_dir+'train_'+str(self.DN_GLOBAL_COUNTER)+'_'+str(frame_index)+'.tif'
            valid_name = self.valid_dir+'valid_'+str(self.DN_GLOBAL_COUNTER)+'_'+str(frame_index)+'.tif'
            inp_train_tiles[0].save(train_name, compression=None, save_all=True, append_images=inp_train_tiles[1:])
            inp_valid_tiles[0].save(valid_name, compression=None, save_all=True, append_images=inp_valid_tiles[1:])
...
def _evalpoly2d(X, Y, c):
    return (c[0] + X*c[1] + Y*c[2] + X*Y*c[3] + X**2*c[4] + Y**2*c[5] + 
           X**2*Y*c[6] + X*Y**2*c[7])
