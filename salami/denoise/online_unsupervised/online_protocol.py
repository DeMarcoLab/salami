...
trainer = Trainer(work_dir, cfg.DN_DEPTH, cfg.DN_NN, device,
                    batchsize=cfg.DN_BATCHSZ, dropout=0.5, lr=0.01, naugment=cfg.DN_NAUGMENT)
...
for index in tqdm.tqdm(index_list):
    ...
    noisy_imgs =     [img_prefix + index2str(index-1) + img_fast_suffix]
    noisy_imgs.append(img_prefix + index2str(index+1) + img_fast_suffix)
    aligned_ref, aligned_imgs = aligner.align_images(noisy_ref, noisy_imgs, align_patch=cfg.AL_PATCH)
    train_ref, valid_ref, train_inp, valid_inp = aligner.generate_denoising_tiles(aligned_ref, aligned_imgs)
...
for i in range(5):
    ...
    trainer.reconstruct(noisy_ref, noisy_imgs, 'test_img_'+str(counter)+'_denoised.tif')
