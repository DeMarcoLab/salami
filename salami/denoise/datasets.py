...
class CustomDataset(Dataset):
    ...
    def __getitem__(self, absolute_img_index):
        ...
        img.seek(index)
        target = np.array(img.getdata(), dtype=np.float32).reshape(
            cfg.DN_TILESZ, cfg.DN_TILESZ
        )
        img.close()
        ...
        # Input images
        inps = np.ndarray(
            (cfg.DN_SUBTILESZ, cfg.DN_SUBTILESZ, self.NN), dtype=np.float32
        )
        for i in range(self.NN):
            img = Image.open(self.img_stacks[stack_index][i])
            img.seek(index)
            inp = np.array(img.getdata(), dtype=np.float32).reshape(
                cfg.DN_TILESZ, cfg.DN_TILESZ
            )
            img.close()
            # translation
            ...
            # contrast
            if self.augment:
                inp *= 1.0 + np.random.randint(-5, 6) / 5.0
            # rotation
