...
class Frame:
    def __init__(self, nx=-1, ny=-1, filename=None):
        ...
        self.D = pyfftw.zeros_aligned(
            (nx, ny), dtype=fftw_float_type, order="C", n=None
        )
        self.FD = pyfftw.zeros_aligned(
            (nx // 2 + 1, ny), dtype=fftw_cmplx_type, order="C", n=None
        )
        self.fwdt = pyfftw.FFTW(
            self.D,
            self.FD,
            axes=(1, 0),
    ...
    def read(self, filename=None):
        if self.inFourierDomain:
            print(
                "Frame object is being read (real space) while in the Fourier Domain: "
                + filename
            )
            sys.exit(-1)
        img = Image.open(filename)
    ...
    def write(self, filename, mode="float32"):
        if self.inFourierDomain:
            print(
                "Frame object is being written (real space) while in the Fourier Domain: "
                + filename
            )
            sys.exit(-1)
    ...
    def normalize(self, border=0):
        if self.inFourierDomain:
            print(
                "Frame object is being normalized in real space while in the Fourier Domain"
            )
            sys.exit(-1)
        if border <= 1:
            self.D -= self.D.mean()
    ...
    def calc_mean(self, border=0):
        if self.inFourierDomain:
            ...
            if border > 0:
                ...
                if n > 1:
                    mean = self.D[
                        border : self.nx - border, border : self.ny - border
                    ].mean()
                else:
                    mean = 0.0
            else:
        ...
        if self.inFourierDomain:
            ...
            if mean == None:
                if nborder < self.n:
                    var = (
                        np.sum(
                            self.D[border : self.nx - border, border : self.ny - border]
                            ** 2
                        )
                        / nborder
                    )
    ...
    def get_best_shift(
        self, target, normalized=False, peakInterpolation=False, mask=None
    ):
        ...
        coords = np.unravel_index(
            np.argmax(
                target.D[
                    center[0] - b[0] : center[0] + b[0] + 1,
                    center[1] - b[1] : center[1] + b[1] + 1,
                ],
                axis=None,
            ),
        ...
        for j in range(cy - radiusk, cy + radiusk + 1):
            ...
            for i in range(radiush + 1):
                ...
                if gsq < ghpsq:
                    ...
                    if gsq <= ghplimsq:
                        self.FD[i, j] *= 0.5 * (
                            1.0 - math.cos(np.pi * (math.sqrt(gsq) - ghp) / gw)
                        )
            ...
            for j in range(nk):
                ...
                for i in range(nh):
                    ...
                    if gsq < glowlimsq:
                        ...
                        FD[i, j] *= math.cos(
                            quarterpi * (1.0 + (math.sqrt(gsq) - glp) / gw)
                        )
